...
import { ICONS, MISSION_CATEGORIES, ATTRIBUTE_MAP } from "@/constants/gameConfig"; // ⚔️ Added ATTRIBUTE_MAP import
...
export default function MissoesClient({ initialMissions, valentes }: any) {
  /* Sorting of mission data by creation timestamp for chronological display */
  ...
  /* Management of the mission archival process and undo functionality */
  const [missionToConfirm, setMissionToConfirm] = useState<any>(null);
  const [showUndo, setShowUndo] = useState(false);
  const [pendingDelete, setPendingDelete] = useState<any>(null);
  const deleteTimerRef = useRef<NodeJS.Timeout | null>(null);

  /* State tracking for the reward allocation modal */
  ...
  /* Filtering logic for the primary mission list based on text input */
  ...
  /* Character list filtering for the reward selection interface */
  ...
  return (
    <main className="min-h-screen py-6 px-4 md:px-8 max-w-7xl mx-auto text-white font-barlow overflow-x-hidden relative">
      ...
      {/* Header section with title and creation link */}
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div className="title-block">
          <h1 className="hud-title-lg text-white m-0 uppercase tracking-widest">Missões Disponíveis</h1>
      ...
      {/* Restricted-width search bar with tactical insignia */}
      ...
      {/* Grid rendering for categorized mission items */}
      <div className="pb-20">
        {missions.length === 0 ? (
...
/**
 * Visual card sub-component for individual mission data
 */
function MissionCard({ mission, onDelete, onOpenModal }: any) {
  ...
  return (
    <div 
      className={`p-6 rounded-2xl flex flex-col border transition-all shadow-xl group relative overflow-hidden backdrop-blur-xl ${theme.border}`}
      style={{
